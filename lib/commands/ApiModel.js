...
pub.deleteResource = function deleteResource(event, context, eventParams, callback) {
    CloudFormationResourceTracker.get(event, context, function (getError, apiModelEntry) {
        ...
        if (apiModelEntry) {
            ApiModelService.deleteModel(apiModelEntry.resourceIdentifier, eventParams.params.restApiId, function (deleteError) {
                if (deleteError) {
...
pub.updateResource = function updateResource(event, context, eventParams, callback) {
    CloudFormationResourceTracker.get(event, context, function (getError, apiModelEntry) {
        ...
        if (apiModelEntry) {
            ApiModelService.patchModel(apiModelEntry.resourceIdentifier, eventParams.params.restApiId, eventParams, function (patchError) {
                if (patchError) {
