...
var ApiDeployEvent = require('./ApiDeployEvent');
...
function patchStage(eventParams, callback) {
    ...
    var params = {
        stageName: eventParams.stageName,
        restApiId: eventParams.restApiId,
        patchOperations: patchOperations
    };
    awsApiGateway.updateStage(params, function (error, apiStage) {
        if (error) {
            logger.log('Error ApiDeployService::patchStage', { error: error, params: params });
        }
        return callback(error, apiStage);
    });
}
