...
pub.getParameters = function getParameters(event) {
    ...
    if (event.RequestType !== 'Delete') {
        ...
        if (!eventParams.params.authorizerUri) {
            throw new Error('Missing parameter {authorizerUri} in input');
        }
        if (!eventParams.params.identitySource) {
            throw new Error('Missing parameter {identitySource} in input');
        }
        if (!eventParams.params.name) {
            throw new Error('Missing parameter {name} in input');
        }
    }
