...
pub.getValidatedCorsConfigFromEventInput = function (eventParams) {
    if (eventParams.params.corsConfig) {
        if (!eventParams.params.corsConfig.allowMethods) {
            throw new Error('Missing parameter {corsConfiguration.allowMethods} in input');
        } else if (eventParams.params.corsConfig.allowMethods === '*') {
            eventParams.params.corsConfig.allowMethods = Constants.CORS_ALL_METHODS;
        }
