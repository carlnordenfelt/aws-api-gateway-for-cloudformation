...
var DEFAULT_RETRY_INTERVAL_MILLIS = 500;
var MAX_RETRIES = 32;
var MAX_RETRY_WINDOW_MILLIS = 5000;

function getWrappedService(sdkOptions, options) {
    _minRetryIntervalMs = options && options.minRetryIntervalMs || DEFAULT_RETRY_INTERVAL_MILLIS;
    _maxRetries = options && options.maxRetries || MAX_RETRIES;
    _retryWindow = options && options.retryWindow || MAX_RETRY_WINDOW_MILLIS;
...
function _isRetryableErrorMessage(message) {
    return message && message.indexOf('Please try again later') > -1;
}

function _isRetryableError(error) {
    return error
        && (error.code === 'TooManyRequestsException'
        || _isRetryableErrorMessage(error.message))
        || error.retryable;
}
