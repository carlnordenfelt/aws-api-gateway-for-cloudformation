...
var ApiResourceEvent = require('./api-resource-event');
var Constants = require('../constants2');
var CorsService = require('../Cors/cors-service');
...
pub.getForResponse = function getForResponse(restApiId, resourceId, callback) {
    ...
    awsApiGateway.getResource(params, function (error, apiDomainName) {
        if (error) {
            logger.log('Error ApiResourceService::getForResponse', { error: error, params: params });
        }
...
/* eslint max-params: 0 */
pub.createResource = function createResource(restApiId, parentResourceId, pathPart, corsConfiguration, callback) {
    var params = {
        restApiId: restApiId,
    ...
    awsApiGateway.createResource(params, function (createError, resource) {
        if (createError) {
            logger.log('Error ApiResourceService::createResource', { error: createError, params: params });
            return callback(createError);
        }
        if (corsConfiguration) {
            CorsService.putOptionsMethod(restApiId, resource.id, corsConfiguration, function (error) {
                if (error) {
...
pub.deleteResource = function deleteResource(resourceId, restApiId, callback) {
    ...
    awsApiGateway.deleteResource(params, function (error) {
        if (error && error.code !== 'NotFoundException') {
            logger.log('Error ApiResourceService::deleteResource', { error: error, params: params });
            return callback(error);
        }
        return callback();
    });
...
pub.patchResource = function patchResource(resourceId, restApiId, eventParams, callback) {
    ...
    awsApiGateway.updateResource(params, function (error, apiResource) {
        if (error) {
            logger.log('Error ApiResourceService::patchResource', { error: error, params: params });
            return callback(error);
        }
...
pub.getResource = function getResource(parameters, position, callback) {
    ...
    awsApiGateway.getResources(params, function (getResourcesError, response) {
        if (getResourcesError && getResourcesError.code !== 'NotFoundException') {
            logger.log('Error ApiResourceService::getResource', { error: getResourcesError, params: params });
